...
'''
 @brief Class for handling the Point Grey camera.
 	This class depends on the pyflycapture2 library made by Robert Jordens <jordens@phys.ethz.ch>.
 	See HOWTO.txt on how to install pyflycapture2.
 	fc2 enums are found in flycapture2_enums.pxi
 	The enums are primarily used to set format7 settings, video mode and frame rate.
...
'''
class PtGreyJordens(object):
	_PtGrey_lib_tag = 'Jordens' # Static tag for checking which library this is
	...
	def RetrieveBufferThread(self):
		...
		self.__error_flag = False
		try:
			self.__fc2_camera.retrieve_buffer(self.__frame_object)
	...
	def StartCapture(self):
		...
		'''
		self.AssertCameraConnected()
		self.__fc2_camera.start_capture()
	...
	def StopCapture(self):
		...
		self.AssertCameraConnected()
		try:
			self.__fc2_camera.stop_capture()
		except Exception, err:
			warnings.simplefilter('always')
			warnings.warn(str(err), Warning)
			warnings.simplefilter('default')
	...
	def GetCameraInfo(self):
		...
		'''
		self.AssertCameraConnected()
		return self.__fc2_camera.get_camera_info()
	...
	def SetTrigger(self, gpio, on_off=True, polarity=None, mode=None):
		'''
		 ...
		 @return trigger_info
		'''
		trigger_info = self.__fc2_camera.get_trigger_mode()
		...
		self.__fc2_camera.set_trigger_mode(**trigger_info)
		return trigger_info
	...
	def SetConfiguration(self, 	num_buffers=None, \
		...
		'''
		self.AssertCameraConnected()
		configurations = self.__fc2_camera.get_configuration()
		...
		if not(self.__capturing):
			self.__fc2_camera.set_configuration(**configurations)
		else:
	...
	def SetFormat7Configuration(self, mode=None, offset_x=None, offset_y=None, width=None, height=None, pixel_format=None):
		...
		'''
		self.AssertCameraConnected()
		format7config = self.__fc2_camera.get_format7_configuration()
		if not(mode == None):
		...
		if not(self.__capturing):
			self.__fc2_camera.set_format7_configuration(**format7config)
		else:
	...
	def SetVideoModeAndFrameRate(self, video_mode=None, frame_rate=None):
		...
		'''
		self.AssertCameraConnected()
		pre_vid_mode, pre_frame_rate = self.__fc2_camera.get_video_mode_and_frame_rate()
		if not(video_mode == None):
			pre_vid_mode = video_mode
		if not(frame_rate == None):
			pre_frame_rate = frame_rate
		if not(self.__capturing):
			self.__fc2_camera.set_video_mode_and_frame_rate(pre_vid_mode, pre_frame_rate)
		else:
			warnings.warn('Cannot set video mode and frame rate while capturing.', UserWarning)
		return self.__fc2_camera.get_video_mode_and_frame_rate()
	...
	def SetFrameRate(self, frame_rate=None):
		...
		'''
		frame_rate_property = self.__fc2_camera.get_property(self.__fc2.FRAME_RATE)
		if frame_rate != None:
			frame_rate_property['abs_value'] = float(frame_rate)
		self.__fc2_camera.set_property(**frame_rate_property)
		return frame_rate_property
	...
	def SetGain(self, gain=None, auto=False):
		...
		'''
		self.AssertCameraConnected()
		prop = self.__fc2_camera.get_property(self.__fc2.GAIN)
		prop['auto_manual_mode'] 	= int(auto)
		prop['abs_control'] 		= int(True)
		if gain != None:
			prop['abs_value'] 		= gain
		self.__fc2_camera.set_property(**prop)
		return prop
	...
	def SetShutter(self, shutter=None, auto=False):
		...
		'''
		self.AssertCameraConnected()
		prop = self.__fc2_camera.get_property(self.__fc2.SHUTTER)
	...
	def SetBrightness(self, brightness=None):
		...
		'''
		self.AssertCameraConnected()
		prop = self.__fc2_camera.get_property(self.__fc2.BRIGHTNESS)
		prop['abs_control'] 		= int(True)
		if brightness != None:
	...
	def SetAutoExposure(self, exposure=None, auto=False):
		...
		'''
		self.AssertCameraConnected()
		prop = self.__fc2_camera.get_property(self.__fc2.AUTO_EXPOSURE)
		prop['on_off'] 				= int(True)
		prop['auto_manual_mode'] 	= int(auto)
		prop['abs_control'] 		= int(True)
		if exposure != None:
	...
	def SetSharpness(self, sharpness=None, auto=False):
		...
		'''
		self.AssertCameraConnected()
		prop = self.__fc2_camera.get_property(self.__fc2.SHARPNESS)
		prop['on_off'] 				= int(True)
		prop['auto_manual_mode'] 	= int(auto)
		prop['abs_control'] 		= int(True)
		if sharpness != None:
	...
	def SetGamma(self, gamma=None, auto=False):
		...
		'''
		self.AssertCameraConnected()
		prop = self.__fc2_camera.get_property(self.__fc2.GAMMA)
		prop['on_off'] 				= int(True)
		prop['auto_manual_mode'] 	= int(auto)
		prop['abs_control'] 		= int(True)
		if gamma != None:
